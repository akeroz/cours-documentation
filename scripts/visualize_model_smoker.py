"""
Script simple pour visualiser le modèle smoker
...
def load_model_and_data():
    ...
    feature_names = metadata['smoker']['feature_names']
    
    # Charger le modèle
    model_path = base_dir / "models" / "model_smoker.pkl"
...
def plot_feature_importance(model, X, output_dir):
    ...
    plt.barh(range(len(indices)), importances[indices], color='coral', alpha=0.7)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'smoker_feature_importance.png', dpi=200, bbox_inches='tight')
    plt.close()
...
def plot_confusion_matrix(model, X, y, output_dir):
    ...
    plt.title('Matrice de Confusion\n(Modèle: smoker)', 
              fontsize=14, fontweight='bold', pad=20)
    plt.tight_layout()
    plt.savefig(output_dir / 'smoker_confusion_matrix.png', dpi=200, bbox_inches='tight')
    plt.close()
...
def plot_prediction_distribution(model, X, y, output_dir):
    ...
    ax1.hist(y_proba[y==0], bins=30, alpha=0.6, label='Non-fumeur', color='blue')
    ax1.hist(y_proba[y==1], bins=30, alpha=0.6, label='Fumeur', color='orange')
    ...
    ax2.boxplot(data_to_plot, labels=['Non-fumeur', 'Fumeur'])
    ...
    plt.suptitle('Analyse des Prédictions - smoker', 
                 fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig(output_dir / 'smoker_predictions.png', dpi=200, bbox_inches='tight')
    plt.close()
...
def plot_top_features_comparison(model, X, y, output_dir):
    ...
    plt.suptitle('Top 5 Features Importantes par Classe\n(Modèle: smoker)', 
                 fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig(output_dir / 'smoker_top_features.png', dpi=200, bbox_inches='tight')
    plt.close()
...
def main():
    """Fonction principale"""
    print("=" * 60)
    print("VISUALISATION MODELE: smoker")
    print("=" * 60)
    
    # Créer le répertoire de sortie
    base_dir = Path(__file__).parent.parent
    output_dir = base_dir / "docs" / "visualizations" / "model_smoker"
